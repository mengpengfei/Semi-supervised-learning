...
from semilearn.nets.cbc.model_cbc_no_lstm import CBCNet_head,CBCNet
...
class ABCNet(nn.Module):
    def __init__(self, backbone, num_classes):
        ...
        self.aux_classifier = nn.Sequential(
            CBCNet_head(config.timestamp,num_classes)
        )
        # self.aux_classifier = self.aux_classifier.half()

    def forward(self, x, **kwargs):
        results_dict = self.backbone(x, **kwargs)
        results_dict['logits_aux'] = self.aux_classifier(results_dict['feat']).permute(1,0,2)[:,-1,:]
        return results_dict
...
class ABC(ImbAlgorithmBase):
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None, **kwargs):
        ...
        lb_class_dist = np.array(lb_class_dist)
        valid = lb_class_dist > 0
        class_weights = np.ones_like(lb_class_dist, dtype=np.float32)
        class_weights[valid] = np.min(lb_class_dist[valid]) / lb_class_dist[valid]
        # For zero‑count classes, you can set weight = 0 (ignore them) or = 1 (no correction)
        class_weights[~valid] = 0.0   # or 1.0
        self.lb_class_dist = torch.from_numpy(class_weights)
    ...
    def train_step(self, *args, **kwargs):
        ...
        # get logits
        logits_x_lb = self.model.module.aux_classifier(feats_x_lb).permute(1,0,2)[:,-1,:]
        logits_x_ulb_s = self.model.module.aux_classifier(feats_x_ulb_s).permute(1,0,2)[:,-1,:]
        with torch.no_grad():
            logits_x_ulb_w = self.model.module.aux_classifier(feats_x_ulb_w).permute(1,0,2)[:,-1,:]
